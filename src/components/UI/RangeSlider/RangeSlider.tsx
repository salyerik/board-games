...
const RangeSlider: FC<iRangeSlider> = ({ range, valueRange, setFromRange, setToRange }) => {
	const progressRef = useRef<HTMLDivElement>(null)

	useEffect(() => {
		if (progressRef.current) {
			progressRef.current.style.left =
				(+valueRange.from / (range.minMaxRange[1] - 1)) * 100 - (range.minMaxRange[1] < 10 ? 20 : 0) + '%'

			progressRef.current.style.right =
				100 - (+valueRange.to / (range.minMaxRange[1] - 1)) * 100 + (range.minMaxRange[1] < 10 ? 20 : 0) + '%'
		}
	}, [valueRange])
	...
	return (
		<div draggable={false} className={s.slider}>
			...
			<div draggable={false} className={s.rangeInputs}>
				<input
					type='range'
					min={range.minMaxRange[0]}
					max={range.minMaxRange[1]}
					step={range.step}
					value={valueRange.from === '' ? 0 : valueRange.from}
				...
				<input
					...
					max={range.minMaxRange[1]}
					step={range.step}
					value={valueRange.to === '' ? 0 : valueRange.to}
